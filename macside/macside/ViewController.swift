...
class ViewController: NSViewController, CBPeripheralManagerDelegate  {
    ...
    //ibeacon    var proximityUUID: NSUUID
    private var viewModel: ViewControllerVMProtocol?
    ...
    func startBoarding(){
        viewModel?.uuid = IBeacon.defaultUUID
               viewModel?.power = IBeacon.defaultPower
               viewModel?.major = IBeacon.defaultMajor
               viewModel?.minor = IBeacon.defaultMinor
               viewModel?.pressedAdvertiseButton()
               updateActiveState()
    }
    
    func setViewModel(viewModel: ViewControllerVMProtocol) {
    ...
    func peripheralManager(_ peripheral: CBPeripheralManager, central: CBCentral, didSubscribeTo characteristic: CBCharacteristic) {
        ...
        if characteristic.uuid.uuidString == C001_CHARACTERISTIC {
            do {
                let data =  "連接成功".data(using: .utf8)
                self.textView.string =  "連接成功"
    ...
    func peripheralManager(_ peripheral: CBPeripheralManager, didReceiveWrite requests: [CBATTRequest]) {
        ...
        if at.characteristic.uuid.uuidString == C001_CHARACTERISTIC {
            ...
            // 收到的原始資料型態為 Data
            if String(data:data,encoding:.utf8) == "sleep" {
                app.tosleep()
            }
            else if String(data:data,encoding:.utf8) == "spotifystart"
            {
                app.spotifystartcontroller()
            }
            else if String(data:data,encoding:.utf8) == "spotifyplay"
            {
                app.spotifyplaycontroller()
            }
            else if String(data:data,encoding:.utf8) == "spotifypause"
            {
                app.spotifystopcontroller()
            }
            else{
